...
actor AgricultureVenturePlatform {
  ...
  // Admin-only endpoint to fetch raw amounts from various records
  public query ({ caller }) func getRawAmountsByGroup(groupId : Text, sampleSize : Nat) : async [RawAmountSample] {
    if (not (AccessControl.hasPermission(accessControlState, caller, #admin))) {
      Debug.trap("Unauthorized: Only admins can fetch raw amount samples");
    };
    ...
    let contributionSamples = List.map<Contribution, RawAmountSample>(
      List.take<Contribution>(groupContributions, sampleSize),
      func(contribution) {
        {
          id = contribution.id;
    ...
    let expenseSamples = List.map<Expense, RawAmountSample>(
      List.take<Expense>(groupExpenses, sampleSize),
      func(expense) {
        {
          id = expense.id;
          recordType = "expense";
    ...
    let revenueSamples = List.map<Revenue, RawAmountSample>(
      List.take<Revenue>(groupRevenues, sampleSize),
      func(revenue) {
        {
          id = revenue.id;
          recordType = "revenue";
    ...
    List.toArray(List.append(List.append(contributionSamples, expenseSamples), revenueSamples));
  };
