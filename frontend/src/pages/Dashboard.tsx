                ...
                <CardContent>
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{group.currency} {centsToDisplayUnits(Number(group.monthlyContributionAmount)).toFixed(2)}</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
...
function MyReturnsCard({ groupId, groupName, currency }: { groupId: string; groupName: string; currency: string }) {
  ...
  const totalContributed = centsToDisplayUnits(Number(returns.totalContributed));
  const totalReceived = centsToDisplayUnits(Number(returns.totalReceived));
  const netProfit = centsToDisplayUnits(Number(returns.netProfit));
