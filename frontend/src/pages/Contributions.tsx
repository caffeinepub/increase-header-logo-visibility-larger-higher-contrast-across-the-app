...
export default function Contributions() {
  ...
  // Calculate totals: amounts are already normalized to cents by useQueries
  const paidCents = myContributions.filter(c => c.status === 'paid').map(c => Number(c.amount));
  const totalPaid = centsToDisplayUnits(sumCentsNumeric(paidCents));

  const pendingCents = myContributions.filter(c => c.status === 'pending').map(c => Number(c.amount));
  const totalPending = centsToDisplayUnits(sumCentsNumeric(pendingCents));

  const overdueCents = myContributions.filter(c => c.status === 'overdue').map(c => Number(c.amount));
  const totalOverdue = centsToDisplayUnits(sumCentsNumeric(overdueCents));
                    ...
                    {myContributions.map((contribution) => (
                      <TableRow key={contribution.id}>
                        <TableCell className="font-medium">
                          {selectedGroup?.currency || 'EUR'} {centsToDisplayUnits(Number(contribution.amount)).toFixed(2)}
                        </TableCell>
